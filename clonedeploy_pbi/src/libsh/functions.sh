...
getZFSTank() {
  ...
  do
     if [ -d "${_chkDir}" ] ; then
        _chkDir="$(realpath ${_chkDir} 2>/dev/null)"
     fi
...
createZFSDataset()
{
	...
	local zds="$(getZFSDataset "${mp}")"
	echo "createZFSDataset: zds = \"${zds}\""

	local zdsrp="$(zfs list -H|grep -w "${zds}"|awk '{ print $5}'|grep -w "^${zds}$")"
	echo "createZFSDataset: zdsrp = \"${zdsrp}\""

	local diff="${mp##${zdsrp}}"
	echo "createZFSDataset: diff = \"${diff}\""

	echo "createZFSDataset: zfs create -o mountpoint=\"${mp}\" -p \"${newds}\""
	zfs create -o mountpoint="${mp}" -p "${newds}"

	local newzds="$(getZFSDataset "${mp}")"
	local bn="$(basename "${newzds}")"
	local zp="${newzds%%/${bn}}"
