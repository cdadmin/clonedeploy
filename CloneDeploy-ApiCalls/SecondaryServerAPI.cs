...
namespace CloneDeploy_ApiCalls
{
    public class SecondaryServerAPI : BaseAPI
    {
        ...
        public SecondaryServerEntity Get(int id)
        {
            ...
            return _apiRequest.Execute<SecondaryServerEntity>(Request);
        }

        public List<SecondaryServerEntity> Get(int limit, string searchstring)
        {
            Request.Method = Method.GET;
            Request.Resource = string.Format("api/{0}/Get", Resource);
            Request.AddParameter("limit", limit);
            Request.AddParameter("searchstring", searchstring);
            var result = _apiRequest.Execute<List<SecondaryServerEntity>>(Request);
            if (result == null)
                return new List<SecondaryServerEntity>();
            else
                return result;
        }
        ...
        {
            Request.Method = Method.GET;
            Request.Resource = string.Format("api/{0}/GetServerOS", Resource);
