...
{
    public class ScriptAPI : BaseAPI
    {
        private readonly ApiRequest _apiRequest;

        public ScriptAPI(string resource) : base(resource)
        {
        ...
        public ScriptEntity Get(int id)
        {
            ...
            return _apiRequest.Execute<ScriptEntity>(Request);
        }

        public List<ScriptEntity> Get(int limit, string searchstring)
        {
            Request.Method = Method.GET;
            Request.Resource = string.Format("api/{0}/Get", Resource);
            Request.AddParameter("limit", limit);
            Request.AddParameter("searchstring", searchstring);
            var result = _apiRequest.Execute<List<ScriptEntity>>(Request);
            if (result == null)
                return new List<ScriptEntity>();
            else
                return result;
        }
        ...
        public ActionResultDTO Post(ScriptEntity tObject)
        {
