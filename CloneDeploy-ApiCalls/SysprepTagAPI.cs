...
namespace CloneDeploy_ApiCalls
{
    public class SysprepTagAPI : BaseAPI
    {
        ...
        public SysprepTagEntity Get(int id)
        {
            ...
            return _apiRequest.Execute<SysprepTagEntity>(Request);
        }

        public List<SysprepTagEntity> Get(int limit, string searchstring)
        {
            Request.Method = Method.GET;
            Request.Resource = string.Format("api/{0}/Get", Resource);
            Request.AddParameter("limit", limit);
            Request.AddParameter("searchstring", searchstring);
            var result = _apiRequest.Execute<List<SysprepTagEntity>>(Request);
            if (result == null)
                return new List<SysprepTagEntity>();
            else
                return result;
        }
