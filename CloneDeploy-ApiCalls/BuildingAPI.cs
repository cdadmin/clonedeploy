...
namespace CloneDeploy_ApiCalls
{
    public class BuildingAPI : BaseAPI
    {
        ...
        public BuildingEntity Get(int id)
        {
            ...
            return _apiRequest.Execute<BuildingEntity>(Request);
        }

        public List<BuildingWithClusterGroup> GetAll(int limit, string searchstring)
        {
            Request.Method = Method.GET;
            Request.Resource = string.Format("api/{0}/GetAll", Resource);
            Request.AddParameter("limit", limit);
            Request.AddParameter("searchstring", searchstring);
            return _apiRequest.Execute<List<BuildingWithClusterGroup>>(Request);
        }
