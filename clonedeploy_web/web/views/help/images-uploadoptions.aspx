...
<p>A hard drive can actually have an mbr and a gpt partition table.  Hard drives that have both will not function with CloneDeploy.  
...
<p>By default, when a Block image is uploaded, all ntfs or extfs filesystems that are larger than 5GB are shrunk to the smallest 
    ...
    problems you can disable that here.</p>
...
<p>If you want to control what hard drives or partitions to upload, this is the first step.  Turn this setting On and start an 
    upload task.  Instead of uploading an entire image, it will only upload the schema.</p>
<h3>Use Custom Upload Schema</h3>

<p>If you want to control what hard drives or partitions to upload, this is the second step.  Check this box and a new table will 
    ...
    and partition that was found, simply check or uncheck the one’s you want.  There is also an option for each partition called fixed. 
     ...
     database, the original is never modified.</p>
</asp:Content>
