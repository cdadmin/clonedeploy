...
<p>The name the user will login with.  If using LDAP authentication it must match the value returned from the Authentication Attribute
    assigned in Admin->Security</p>

 
<h3>Role</h3>

<p>There are two types of Roles.  Administrators and Users.  Administrators have complete control over the system and cannot be 
    controlled by ACLs.  Users have as much power as you want to give them using ACL’s.  By default a new user only has search 
    permissions, and cannot really do anything until more permissions are applied.</p>
...
<p>This is authorization key that is returned when logging in to the client boot image.  It is used to Authorize calls to the 
    client web service.  If you feel that a user’s token is ever compromised you can generate a new one.</p>
