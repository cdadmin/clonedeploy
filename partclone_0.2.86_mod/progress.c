...
extern void progress_update(struct progress_bar *prog, unsigned long long copied, unsigned long long current, int done)
{
	...
	fprintf(fp, _("*%s*%s*%6.2f%%*%6.2f%s/min"), prog_stat.Eformated, prog_stat.Rformated, prog_stat.percent, prog_stat.speed, prog_stat.speed_unit);
	...
	fprintf(fp, _("*%s*%s*100.00%%*%6.2f%s/min"), prog_stat.Eformated, prog_stat.Rformated, prog_stat.speed, prog_stat.speed_unit);
	if((prog->flag == IO) || (prog->flag == NO_BLOCK_DETAIL))
