...
int main(int argc, char **argv) {
	...
	if (opt.clone) {
		...
		volume_size = (unsigned long long)((image_hdr.block_size+sizeof(unsigned long))*image_hdr.totalblock);
		if (opt.used_block_size) {
			log_mesg(0, 0, 1, debug, "\nused_size:%llu", print_size(needed_size, MBYTE));
			log_mesg(0, 0, 1, debug, "\nvolume_size:%llu \n", print_size(volume_size, MBYTE));
			exit(0);
		}
