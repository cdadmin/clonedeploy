...
*/
/*
* PXESERVA/SYSLINUX/PXELINUX functions
* Patrick Masotta (c) 2012
* Serva 2.0
...
function syslinux_sha1(s) {
    var salt = Math.random().toString(36).substring(6);
    salt += Math.random().toString(36).substring(6);
    salt = salt.substring(0, 4).toUpperCase() + salt.substring(4, 8);
    if (s == "") return "Error: Empty password";
    if (/[\x00-\x1F]/.test(str2rstr_utf8(s))) return "Error: Non-printable ASCII char/s";
    if (/[\x7F-\xFF]/.test(str2rstr_utf8(s))) return "Error: Non-ASCII char/s";
    return "$4$" + salt + "$" + rstr2b64(rstr_sha1(salt + str2rstr_utf8(s))) + "$";
}
